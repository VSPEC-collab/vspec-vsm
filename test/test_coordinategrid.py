...
def test_coodinategrid_typeerror():
    ...
    with pytest.raises(TypeError):
        _ = CoordinateGrid(Nlat=100, Nlon=200.0)
    with pytest.raises(TypeError):
        _ = CoordinateGrid(Nlat=100.0, Nlon=200)
...
def test_coordinategrid_eq():
    ...
    """
    grid1 = CoordinateGrid(Nlat=100, Nlon=200)
    grid2 = CoordinateGrid(Nlat=100, Nlon=200)
    assert grid1 == grid2

    with pytest.raises(TypeError):
        _ = grid1 == 1

    grid3 = CoordinateGrid(Nlat=101, Nlon=200)
    assert grid1 != grid3

    grid4 = CoordinateGrid(Nlat=200, Nlon=100)
    assert grid1 != grid4
